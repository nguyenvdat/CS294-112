...
class ModelBasedPolicy(object):
    ...
    def _setup_training(self, state_ph, next_state_ph, next_state_pred):
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        norm_state_diff = utils.normalize(next_state_ph - state_ph, self._init_dataset.state_mean, self._init_dataset.state_std)
        norm_pred_state_diff = utils.normalize(next_state_pred - state_ph, self._init_dataset.state_mean, self._init_dataset.state_std)
        # loss = tf.reduce_mean(tf.squared_difference(norm_state_diff, norm_pred_state_diff))
        loss = tf.losses.mean_squared_error(norm_state_diff, norm_pred_state_diff)
    ...
    def train_step(self, states, actions, next_states):
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        # eval1 = self._sess.run(self._loss, feed_dict={self._state_ph:states, self._action_ph:actions, self._next_state_ph:next_states})
        # print(eval1)
