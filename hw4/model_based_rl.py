    ...
    def run_q1(self):
        ...
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            ...
            for i, action in enumerate(actions):
                pred_state = self._policy.predict(current_state, action)
                pred_states.append(pred_state)
                current_state = pred_state
                if i < len(actions) - 1:
                    print(np.mean((states[i + 1] - pred_state) ** 2))
            print('')
            states = states[1:300]
            pred_states = pred_states[0:299]
            states = np.asarray(states)
            pred_states = np.asarray(pred_states)
            # print(np.mean((states[:3] - pred_states[:3]) ** 2))
            # print(pred_states[0:2])
            # print(states[0:2])
            # print("")
