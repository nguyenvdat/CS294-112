...
class Exemplar(Density_Model):
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.Variable(np.zeros(int(z_size)), dtype=tf.float32)
    ...
    def update(self, state1, state2, target):
        ...
        self.sess.run(self.update_op, feed_dict={self.state1: state1, self.state2: state2, self.discrim_target: target})
        ll, kl, elbo = self.sess.run([self.log_likelihood, self.kl, self.elbo], feed_dict={self.state1: state1, self.state2: state2,   self.discrim_target: target})
        return ll, kl, elbo
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run(self.discriminator.probs, feed_dict={self.state1: state1, self.state2: state2})
        return likelihood
